...
export const conflictResolutionOutputSchema = z.object({
  analysis: z.string().min(10).max(1000),
  talkingPoints: z.array(z.string().min(5).max(300)).min(1).max(10),
  script: z.string().min(10).max(3000),
  deEscalationTips: z.array(z.string().min(5).max(300)).min(1).max(5),
}).strict();

export const microHabitPlanOutputSchema = z.object({
  goalBreakdown: z.string().min(10).max(1000),
  dailyHabits: z.array(
    z.object({
      habit: z.string().min(5).max(100),
      timeEstimate: z.string().min(2).max(50),
      trigger: z.string().min(5).max(200),
    }).strict()
  ).min(1).max(10),
  weeklyReviews: z.array(z.string().min(5).max(300)).min(1).max(5),
  burnoutPrevention: z.string().min(10).max(1000),
}).strict();
...
export const SCHEMA_DESCRIPTIONS = {
  ...
  conflictResolution: `{
  "analysis": "string (brief analysis of the conflict)",
  "talkingPoints": ["point 1", "point 2"],
  "script": "string (word-for-word script to use)",
  "deEscalationTips": ["tip 1", "tip 2"]
}`,
  microHabitPlan: `{
  "goalBreakdown": "string (how the goal is broken down)",
  "dailyHabits": [
    {
      "habit": "string (the habit)",
      "timeEstimate": "string (e.g. 5 mins)",
      "trigger": "string (when to do it)"
    }
  ],
  "weeklyReviews": ["review item 1", "review item 2"],
  "burnoutPrevention": "string (advice on preventing burnout)"
}`
};
